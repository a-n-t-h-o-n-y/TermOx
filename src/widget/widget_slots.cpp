...
sig::Slot<void()> hide(Widget& w) {
    sig::Slot<void()> slot{[&w] { w.set_visible(false); }};
    slot.track(w.destroyed);
    return slot;
}

sig::Slot<void()> show(Widget& w) {
    sig::Slot<void()> slot{[&w] { w.set_visible(true); }};
    slot.track(w.destroyed);
    return slot;
}
...
sig::Slot<void()> set_background(Widget& w, Color c) {
    sig::Slot<void()> slot{[&w, &c] { w.set_background(c); }};
    slot.track(w.destroyed);
    return slot;
}
...
sig::Slot<void()> set_foreground(Widget& w, Color c) {
    sig::Slot<void()> slot{[&w, &c] { w.set_foreground(c); }};
    slot.track(w.destroyed);
    return slot;
}
