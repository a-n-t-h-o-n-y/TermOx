...
void Animation_event_loop::run() {
    fut_ = std::async(std::launch::async, [this] { this->Event_loop::run(); });
}
...
void Animation_event_loop::unregister_widget(Widget& w) {
...
void Animation_event_loop::post_all() {
    ...
    if (time_to_sleep > std::chrono::duration::zero) {
