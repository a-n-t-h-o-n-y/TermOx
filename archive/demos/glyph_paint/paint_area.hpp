   ...
   public:
    ...
    void set_symbol(ox::Glyph s)
    {
        ...
        if (s.brush.background != ox::Color::Background)
            current_glyph_ = current_glyph_ | bg(s.brush.background);
        if (s.brush.foreground != ox::Color::Foreground)
            current_glyph_ = current_glyph_ | fg(s.brush.foreground);
        glyph_changed(current_glyph_);
    }

    void set_foreground_color(ox::Color c)
    {
        current_glyph_ = current_glyph_ | fg(c);
        if (!erase_enabled_)
            glyph_changed(current_glyph_);
    }
    ...
    {
        current_glyph_ = current_glyph_ | bg(c);
        if (!erase_enabled_)
            glyph_changed(current_glyph_);
    }
    ...
    {
        current_glyph_ = current_glyph_ | t;
        if (!erase_enabled_)
            glyph_changed(current_glyph_);
    }
